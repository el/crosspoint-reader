  ...
  for (int bmpY = 0; bmpY < (bitmap.getHeight() - cropPixY); bmpY++) {
    ...
    for (int bmpX = cropPixX; bmpX < bitmap.getWidth() - cropPixX; bmpX++) {
      ...
      if (renderMode == BW && val < 3) {
        ...
        // Factory-waveform modes: the plane bit *is* the level bit, so black (0)
        // leaves both planes cleared and white (3) sets both.
...
void GfxRenderer::displayGrayBuffer(const bool factoryMode) const {
  display.displayGrayBuffer(fadingFix, factoryMode);
  absoluteGrayPlanes = false;
}
